# API CallTracking (выгрузка звонков)  
## Содержание  
...
https://ckct.ru/api/index.php?{{format}}/{{version}}/{{apikey}}/{{entity}}/{{method}}/{{pages}}/{{from}}/{{to}}  
...
#### Пример запроса  
https://ckct.ru/api/index.php?json/v3/x9xx99xx99xxxx99/call/get/0/2020-05-01/2020-05-02  
...
https://ckct.ru/api/index.php?json/v3/99999xxx99xxxx99/call/getByRole/0/2020-05-01/2020-05-02  
Данный запрос демонстрирует запрос на получение звонков, выданных пользователю по роли, в формате JSON с 1 по 2 мая 2020.  
https://ckct.ru/api/index.php?json/v3/x9xx99xx99xxxx99/call/getWithRole/0/2020-05-01/2020-05-02  
...
*tariffTime* — тарифицируемое время разговора в минутах (INT)  
...
*entryPointQuery* – страница входа вместе с get параметрами (STRING)  
...
*ctCallId* – ID звонка в коллтрекинге (INT)  
*ctCampaignId* – ID кампании в коллтрекинге (INT)  
*ctCampaignName* – название кампании (STRING)  
*ctIsListened* – факт прослушивания записи звонка в ЛК ("","0","1")  
*tags* – массив тэгов присвоенных звонку (ARRAY[STRING])  

#### Форматы ответов при неправильном или неуспешном запросе  
В случае если по каким либо причинам запрос был неуспешный, то ответ вернется следующего формата:
```
{
  "inbound": {
    "error": "No rows found"
